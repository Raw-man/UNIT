...
class ThreadPool {
 public:
  ...
  // Waits until threads finish all tasks and shutdowns the pool
  void Shutdown() {
    {
  ...
  class ThreadWorker {
   public:
    ...
    void operator()() {
      ...
      while (!thread_pool->shutdown_requested || !thread_pool->queue.empty()) {
        thread_pool->busy_threads--;
        thread_pool->condition_variable.wait(
            lock, [this] { return this->thread_pool->shutdown_requested || !this->thread_pool->queue.empty(); });
        thread_pool->busy_threads++;
 ...
 public:
  ...
  std::size_t GetNumThreads() const { return threads.size(); }
